...
$st = $db->prepare('SELECT id FROM servicios_respuestas WHERE id = ?');
...
// ── 1. Borrar archivos físicos de adjuntos ────────────────────────────────
$st = $db->prepare('SELECT nombre_archivo FROM adjuntos_servicios WHERE tipo_entidad = "respuesta" AND entidad_id = ?');
$st->bind_param('i', $reply_id);
$st->execute();
$res = $st->get_result();
...
$st = $db->prepare('DELETE FROM adjuntos_servicios WHERE tipo_entidad = "respuesta" AND entidad_id = ?');
$st->bind_param('i', $reply_id);
$st->execute();
$st->close();

// ── 3. Eliminar likes de la respuesta ─────────────────────────────────────
$st = $db->prepare('DELETE FROM respuestas_servicios_likes WHERE id_respuesta = ?');
$st->bind_param('i', $reply_id);
$st->execute();
$st->close();

// ── 4. Eliminar la respuesta ──────────────────────────────────────────────
$st = $db->prepare('DELETE FROM servicios_respuestas WHERE id = ?');
$st->bind_param('i', $reply_id);
$st->execute();
$st->close();
