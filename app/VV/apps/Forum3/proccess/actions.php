...
case "topicPending":
  ...
  // Dropzone puede mandar 'adjuntos' o 'adjuntos[]' o variantes
  $filesKey = null;
  foreach (array_keys($_FILES) as $k) {
    if (strpos($k, 'adjuntos') === 0) { $filesKey = $k; break; }
  }

  // helper: flatten recursivo para cualquier forma rara de $_FILES
  $flattenFiles = function($field) {
    $out = [];
    $walk = function($n, $t, $tmp, $e, $s) use (&$out, &$walk) {
      if (is_array($n)) {
        foreach ($n as $k => $v) {
          $walk(
            $n[$k],
    ...
    $walk($field['name'], $field['type'], $field['tmp_name'], $field['error'], $field['size']);
    return $out;
  };

  if ($filesKey && isset($_FILES[$filesKey]) && !empty($_FILES[$filesKey]['name'])) {

    $uploadDir = dirname(__DIR__) . '/uploads/adjuntos/'; // .../Forum/uploads/adjuntos/
    if (!is_dir($uploadDir)) { @mkdir($uploadDir, 0755, true); }

    if (!is_dir($uploadDir) || !is_writable($uploadDir)) {
      echo json_encode(['control' => "0", 'error' => "Directorio uploads no existe o no es writable"]);
      break;
    }

    $maxSize = 16 * 1024 * 1024; // 16MB por archivo (tu server puede tener más bajo igual)
    $allowedMimes = [
      'image/jpeg','image/png','image/webp','image/gif',
      'application/pdf',
      'application/msword',
    ...
    $files = $flattenFiles($_FILES[$filesKey]);
    ...
    foreach ($files as $i => $f) {
      ...
      if ($err !== UPLOAD_ERR_OK) {
        echo json_encode(['control' => "0", 'error' => "Error subiendo archivo (code $err): $name"]);
        break 2;
      }

      if ($size > $maxSize) {
        echo json_encode(['control' => "0", 'error' => "Archivo muy grande: $name"]);
        break 2;
      }

      $mime = $finfo->file($tmp);
      if (!in_array($mime, $allowedMimes, true)) {
        echo json_encode(['control' => "0", 'error' => "Tipo no permitido ($mime): $name"]);
        break 2;
      }
      ...
      if (!move_uploaded_file($tmp, $dest)) {
        echo json_encode(['control' => "0", 'error' => "No se pudo guardar el archivo: $name"]);
        break 2;
      }

      $adj = new _Adjuntos();
      $adj->vars['tipo_entidad']    = $reply_id ? 'respuesta' : 'tema';
      $adj->vars['entidad_id']      = $reply_id ? $reply_id : $tema_id;
      $adj->vars['nombre_original'] = $name;
      $adj->vars['nombre_archivo']  = $safeName;
      $adj->vars['extension']       = $ext;
      $adj->vars['mime_type']       = $mime;
      $adj->vars['tamano']          = $size;
      $adj->vars['orden']           = (int)$i;
      $adj->vars['subido_por']      = $_SESSION['user'];
      $adj->vars['estado']          = 1;
      $adj->insert();
